...
// Function to swap two proc_info structs
void 
swap(struct proc_info *a, struct proc_info *b) {
    struct proc_info temp = *a;
    *a = *b;
    *b = temp;
}
// Function to dump process information
void 
    ...
    *size = 0;
    ...
    for (int i = 0; i < NPROC; ++i, ++p) {
        switch (p->state) {
            case RUNNING:
            case RUNNABLE:
                process[*size].memsize = p->sz;
                process[*size].pid = p->pid;
                (*size)++;
                break;
            default:
                // Skipping processes that are not running or runnable
                continue;
        }
